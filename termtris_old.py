...
class Board:
	...
	def _empty_point(self, point):
		self._set_point(point, EMPTY)

	def _fill_point(self, point):
		self._set_point(point, FILLED)

	def _set_point(self, point, char):
		self.state[point.x][point.y = char
	...
	def can_drop(self, block):
		for point in block.current_coords:
			...
			if self.get_state(point_below[0], point_below[1]) != EMPTY:
				return False
		return True
